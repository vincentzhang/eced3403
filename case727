...
int iret; // if instruction is IRET iret=1 otherwise iret=0
...
BYTE sign; //Sign bit for overflow 
BYTE adr1;// address for Irr 
BYTE adr2; //address for Irr 

int c1;// c flag complement for CCF
...
{	
	...
	if (lnib >= 0x08)
	{
	  switch(lnib)
		  ...
		  case 0x0C: /* LD dst, IMM */
               src = prog_mem_fetch();
               reg_mem[RPBLK|hnib].contents= src;    
               break;
		  ...
		  case 0x0E: /* INC dst re r 6*/   
               dst = RPBLK | hnib; 
               sign = SIGN(reg_mem[dst].contents);//check the sign
               reg_mem[dst].contents += 1;
               
			   sign = SIGN(reg_mem[dst].contents); // get the sign 
          ...
          case 0x0F: /* STOP .. NOP */
          	switch (hnib)
               ...
               case 0x0A: /* RET */	
               			pc=reg_mem[stack].contents;//content of stack popped into pc
               			stack+=2;//	
					    sanity+=14;
               break;
			   ...
			   case 0x0B: //IRET   // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1 
               		iret=1;// if interrupt retrun, set iret=1 and do pop which is at the end of function  
               		sanity+=16;
            		reg_mem[IMR] . contents |= INT_ENA; // IMR(7)<-1, reenable the interrup
               		break;
  ...
  /*check low nibblee through 0x00 0x07*/
  else if((lnib>=0x02)&&(lnib<=0x07))
     {
		 ...
		 if((hnib>=0x00 && hnib<=0x07) || (hnib==0X0A)||(hnib==0x0B) )
		{ 						
				...
				switch(hnib)
				{
					...
					case 0x01: /* ADC */ 	
						...
						result=dst+src+C;  
						reg_mem[temp].contents=result;// store the result back to the dst adc
					...
					case 0x03: /* SBC */
						...
						result=dst-src-C;//dst<--dst-src-C
						reg_mem[temp].contents=result;
		 ...
		 else
		{
			...
			{		
				...
				case 0x09:/*LDE Irr, r ; LDEI Irr, Ir */
					switch(lnib)
					{
						case 0x02:/* *LDE Irr, r */
							ds= prog_mem_fetch();
    ...
    {	/* the rest will be the same instruction but R an IR*/
		...
		switch(lnib)
		{
			case 0x00: //R
				dst = prog_mem_fetch();//address
				temp=dst;
				dst = reg_mem[dst].contents;// content
				break; 
			case 0x01: //IR
				dst = prog_mem_fetch();
			...
			switch(hnib)
		{
			...
			case 0x06: /* COM */
				reg_mem[temp].contents= ~dst;//1 complemented
				reg_mem[FLAGS].contents	 = FLAG_V(0);//V alway 	
				sanity+=6; 
				break;
			...
			case 0x07: /* PUSH */  //??????????????????
				src=dst;
				stack-=1;// sp<--sp-1
				reg_mem[stack].contents=src;//sp<--src	
				break;
	
			case 0x0B: /* CLR */
				reg_mem[temp].contents = 0;
				sanity+=6;
				break;
			
			case 0x0D: /* SRA */
				reg_mem[temp].contents = ((dst>>1)|(dst&0x80));
            	reg_mem[FLAGS].contents	 = FLAG_C(dst&0X01);
            	reg_mem[FLAGS].contents	 = FLAG_V(0);
				sanity+=6;
				break;
			
			case 0x0E: /* RR */
				reg_mem[temp].contents = ((dst>>1)|((dst&0x01)<<7));
            	reg_mem[FLAGS].contents	 = FLAG_C(dst&0X01);
				sanity+=6;
				break;
			
			case 0x0F: /* SWAP */
				reg_mem[temp].contents= (LBYTE(dst)<<4 | HBYTE(dst));// dst(0-3)<-->dst(4-7)
				sanity+=8;
			break;
		}
